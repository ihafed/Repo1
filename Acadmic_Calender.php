...
function Acadmic_Calendr_visit()
        {
            visitCount = getCookie("Acadmic_Calendr_visit");
            if (visitCount == "") 
            {
            setCookie("Acadmic_Calendr_visit", 1);
            }
            else 
            {
            visitCount = parseFloat(visitCount)+1;
            setCookie("Acadmic_Calendr_visit", visitCount);
            }
        }
      </script>  
</head>
<body bgcolor="#D3D3D3" onLoad="Greeting(); Acadmic_Calendr_visit();"
LINK="#FFFAFA"�
VLINK="#87CEEB"
ALINK="#FFE4C4"
text="#FFFAFA"
...
<?php
	...
	mysql_select_db("student_services");
       ...
       $res=mysql_query("select * from Calendar ORDER BY id ");

	echo"<table border=1 align=center> 
	<tr><th><font color='#CC9900'><font size= '4'>Event</font></strong></font></th><th><font color='#CC9900'><font size= '4'>Date</font></font></th>";
	...
	echo "<td><font color='#F3E7C2'><center>"; echo $row["event"]; echo "</td>";
	echo "<td><font color='#F3E7C2'><center>"; echo $row["date"]; echo "</td>";
